  ...
  const scanComments = async () => {
    ...
    try {
      ...
      if (data?.hasMore) {
        ...
        // Now call adjudicator and post-processing since all batches are complete
        const needsAdjudication = accumulated.filter(c => 
          (c.scanA_concerning !== c.scanB_concerning) || 
          (c.scanA_identifiable !== c.scanB_identifiable)
        ).length;
        
        if (needsAdjudication > 0) {
          console.log(`[ADJUDICATION] ${needsAdjudication} comments need adjudication`);
          toast.info(`Starting adjudication for ${needsAdjudication} comments...`);
          ...
          try {
            ...
            if (adjudicationResponse.error) {
              console.error('[ADJUDICATION] Error:', adjudicationResponse.error);
              toast.error('Adjudication failed, but scan results are available');
            } else {
              console.log('[ADJUDICATION] Completed successfully');
              if (adjudicationResponse.data?.comments) {
                accumulated = adjudicationResponse.data.comments;
                (data as any).comments = accumulated;
              }
              toast.success('Adjudication completed!');
            }
          } catch (adjudicationError) {
            console.error('[ADJUDICATION] Failed:', adjudicationError);
            toast.error('Adjudication failed, but scan results are available');
          }
        }

        // Now call post-processing
        const concerningComments = accumulated.filter(c => 
          c.final_concerning || c.scanA_concerning || c.scanB_concerning
        );
        const identifiableComments = accumulated.filter(c => 
          c.final_identifiable || c.scanA_identifiable || c.scanB_identifiable
        );
        
        if (concerningComments.length > 0 || identifiableComments.length > 0) {
          console.log(`[POST_PROCESSING] Processing ${concerningComments.length} concerning and ${identifiableComments.length} identifiable comments`);
          toast.info('Starting post-processing (redaction/rephrasing)...');
          
          try {
            const postProcessResponse = await supabase.functions.invoke('post-process-comments', {
              body: {
                comments: accumulated,
                scanRunId: scanRunId
              }
            ...
            if (postProcessResponse.error) {
              console.error('[POST_PROCESSING] Error:', postProcessResponse.error);
              toast.error('Post-processing failed, but scan results are available');
            } else {
              console.log('[POST_PROCESSING] Completed successfully');
              if (postProcessResponse.data?.comments) {
                accumulated = postProcessResponse.data.comments;
                (data as any).comments = accumulated;
              }
              toast.success('Post-processing completed!');
            }
          } catch (postProcessError) {
            console.error('[POST_PROCESSING] Failed:', postProcessError);
            toast.error('Post-processing failed, but scan results are available');
          }
        }
      }
